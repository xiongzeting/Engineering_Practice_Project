"""Generate Chinese markdown report from v3_detection summary.json."""
...
def render(m: dict) -> str:
    ...
    ctrl = m["control"]
    lines: list[str] = []
    lines.append("# v3 错误检测能力评测报告\n")
    lines.append(f"- 数据集：MATH 200 题（DeepSeek v4-pro 检测，二分类 + 错步定位）")
    lines.append(f"- v3 改动：在 v2 prompt 基础上加『变量一致性』专章，主攻 variable 类召回")
